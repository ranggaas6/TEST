...
// @version      2.6.0
// @description  Plugin KissKH — v2.6.0: kkey di-generate via pageEval (context halaman kisskh.co) untuk /api/Sub/ yang validasi ketat
// @author       UserScript
// ==/UserScript==
...
const _log = (msg, type) => { try { _log(msg, type); } catch(e) { console.log('[KissKH]', msg); } };
...
// Headers yang mensimulasikan XHR dari dalam halaman kisskh.co
// Sec-Fetch-* dikirim manual via GM_xmlhttpRequest (tidak bisa dioverride di browser fetch biasa)
// Tanpa Sec-Fetch-Site: same-origin, server KissKH mengembalikan 403 pada endpoint /api/Sub/
const KK_HEADERS_API = {
  'Referer':          KK_BASE + '/',
  'Origin':           KK_BASE,
...
// Header untuk fetch source video (.png endpoint) — sama tapi Accept berbeda
const KK_HEADERS_SRC = Object.assign({}, KK_HEADERS_API, {
...
function _parseBody(res, label) {
  if (!res) { if(label) _log('[KissKH] '+label+': res null','warn'); return null; }
  ...
  if (b === null || b === undefined || b === 'undefined') {
    if(label) _log('[KissKH] '+label+': body null/undefined','warn');
    return null;
  }
  if (typeof b !== 'string') {
    if(label) _log('[KissKH] '+label+': body bukan string: '+typeof b,'warn');
    return null;
  }
  if (b.trim() === '') {
    if(label) _log('[KissKH] '+label+': body string kosong','warn');
    return null;
  }
...
// ── Ambil kkey via pageEval (di context halaman kisskh.co) ───────────────
// Root cause 403 pada /api/Sub/: kkey yang kita generate di blob explorer
// menggunakan window.document.URL = blob URL, navigator.userAgent palsu ('kisskh').
// Server KissKH memvalidasi kkey secara ketat untuk /api/Sub/ — kkey harus
// di-generate dengan nilai asli dari browser (URL drama, userAgent nyata).
// Solusi: jalankan _0x54b991(epsId) tanpa parameter → pakai default dari
// window context halaman kisskh.co yang sebenarnya via API.pageEval.
async function _getKkey(epsId) {
  if (_kkKeyCache.has(epsId)) return _kkKeyCache.get(epsId);

  try {
    // Pastikan common.js sudah di-cache (untuk fallback dan pageEval)
    if (!_commonJsCache) {
      const htmlRes = await API.launcherFetch(KK_BASE, null, {
      ...
      _log('[KissKH] homepage res keys=['+_hKeys.join(',')+'] status='+_hStatus+' body_len='+_hLen, 'info');
      const html = _parseBody(htmlRes, 'fetch KissKH homepage');
      if (!html) throw new Error('Gagal fetch halaman KissKH (body_len='+_hLen+')');
      _log('[KissKH] HTML snippet: '+html.slice(0,200), 'info');

      const allScripts = [...html.matchAll(/src="([^"]*\.js[^"]*)"/g)].map(m=>m[1]);
      _log('[KissKH] Script tags ditemukan: '+allScripts.join(' | '), 'info');
      ...
      _log('[KissKH] Fetch JS: ' + jsUrl, 'info');

      const jsRes = await API.launcherFetch(jsUrl, null, {
        'Referer': KK_BASE + '/',
        'Sec-Fetch-Site': 'same-origin',
      ...
      _log('[KissKH] JS size: ' + jsCode.length + ' chars, punya _0x54b991: ' + jsCode.includes('_0x54b991'), 'info');
      _commonJsCache = jsCode;
    }

    // ── Strategi 1: pageEval — eval di context halaman kisskh.co ──────────
    // Menghasilkan kkey dengan window.document.URL dan navigator asli browser.
    if (typeof API.pageEval === 'function') {
      try {
        const evalCode = _commonJsCache + '\n;(function(){try{return _0x54b991(' + Number(epsId) + ');}catch(e){return "__ERR:"+e.message;}})()';
        const res = await API.pageEval(evalCode);
        const kkey = res && res.result;
        if (kkey && !String(kkey).startsWith('__ERR') && kkey.length > 10) {
          _log('[KissKH] kkey via pageEval OK: ' + String(kkey).slice(0,30) + '...', 'success');
          _kkKeyCache.set(epsId, kkey);
          return kkey;
        }
        _log('[KissKH] pageEval kkey gagal: ' + kkey, 'warn');
      } catch(e) {
        _log('[KissKH] pageEval error: ' + e.message, 'warn');
      }
    }

    // ── Strategi 2: eval di blob (fallback, kkey mungkin hanya valid untuk /Episode/) ──
    _log('[KissKH] Fallback: eval kkey di blob context', 'info');
    const fnName = '_0x54b991';
    ...
    if (!kkey || String(kkey).startsWith('__ERR')) {
      ...
      _log('[KissKH] kkey2 (blob no-param): ' + String(kkey2).slice(0,30), 'info');
      _kkKeyCache.set(epsId, kkey2||'');
      return kkey2||'';
    }

    _log('[KissKH] kkey OK (blob): ' + String(kkey).slice(0,30) + '...', 'success');
    _kkKeyCache.set(epsId, kkey);
    return kkey;

  } catch(e) {
    _log('[KissKH] Gagal getKkey: ' + e.message, 'warn');
    _commonJsCache = null;
    return '';
  }
}
...
async function _fetchSources(epsId, kkItem) {
  ...
  _log('[KissKH] Fetch source: ep=' + epsId + ' kkey=' + (kkey?'OK':'empty'), 'info');

  const res = await API.launcherFetch(url, null, KK_HEADERS_SRC, 'GET');
  ...
  _log('[KissKH] source body_len='+bodyLen+' finalUrl='+finalUrl, bodyLen>0?'info':'warn');
  _log('[KissKH] source body: '+bodyPrev, bodyLen>0?'info':'warn');
  ...
  _log('[KissKH] Source: video=' + (d.Video||'-') + ' 3p=' + (d.ThirdParty||'-'), 'success');
  return { video: d.Video||d.video||null, thirdParty: d.ThirdParty||d.thirdParty||null };
}

// ── Ambil subtitle list ───────────────────────────────────────────────────
// Cloudflare Bot Management memblokir GM_xmlhttpRequest ke /api/Sub/ karena
// TLS fingerprint-nya berbeda dari browser asli. Solusi: gunakan API.pageFetch
// yang menjalankan fetch() native di konteks halaman kisskh.co — membawa
// cf_clearance cookie dan TLS fingerprint Chrome asli.
async function _fetchSubs(epsId) {
  const kkey = await _getKkey(epsId);
  const url  = KK_SUB + epsId + (kkey ? '?kkey=' + kkey : '');
  _log('[KissKH] Fetch subs via pageFetch: ' + url.slice(0,80) + '...', 'info');

  // ── Coba 1: pageFetch (fetch() di konteks halaman kisskh.co) ─────────────
  if (typeof API.pageFetch === 'function') {
    try {
      const res = await API.pageFetch(url, { 'Accept': 'application/json, */*' });
      const body = res && res.body;
      const status = res && res.status;
      _log('[KissKH] Subs pageFetch status=' + status + ' len=' + (body ? body.length : 'null'), 'info');
      if (body && _isJson(body)) {
        const arr = JSON.parse(body);
        const filtered = Array.isArray(arr) ? arr.filter(s => s.src && s.label) : [];
        _log('[KissKH] Subs count: ' + filtered.length + ' ' + filtered.map(s=>s.label).join(', '), filtered.length ? 'success' : 'warn');
        if (filtered.length) return filtered;
      }
    } catch(e) {
      _log('[KissKH] Subs pageFetch error: ' + e.message, 'warn');
    }
  } else {
    _log('[KissKH] API.pageFetch tidak tersedia — update Core ke v10.1.0', 'warn');
  }

  // ── Coba 2: launcherFetch biasa (fallback, mungkin 403 dari Cloudflare) ───
  try {
    const res = await API.launcherFetch(url, null, KK_HEADERS_API, 'GET');
    const body = res && (res.body !== undefined ? res.body : null);
    const status = (res && res.status) || '?';
    _log('[KissKH] Subs launcherFetch status=' + status + ' len=' + (body!=null?String(body).length:'null'), 'info');
    if (body && typeof body === 'string' && _isJson(body)) {
      const arr = JSON.parse(body);
      const filtered = Array.isArray(arr) ? arr.filter(s => s.src && s.label) : [];
      if (filtered.length) return filtered;
    }
  } catch(e) {
    _log('[KissKH] Subs launcherFetch error: ' + e.message, 'warn');
  }

  _log('[KissKH] Subs: tidak ada subtitle ditemukan', 'warn');
  return [];
}


async function _fetchSubContent(src) {
  // Coba native fetch dulu
  try {
    const r = await fetch(src);
    const text = await r.text();
    _log('[KissKH] SubContent native status=' + r.status + ' len=' + text.length, 'info');
    if (text && text.trim()) {
      if (src.includes('.txt')) return await _decryptSubTxt(text);
      return text;
    }
  } catch(e) {
    _log('[KissKH] SubContent native error: ' + e.message, 'warn');
  }
  // Fallback launcherFetch
  try {
...
async function searchKissKH(title, tmdbYear, tmdbType) {
  ...
  try {
    const raw = _parseBody(await API.launcherFetch(KK_SEARCH + encodeURIComponent(title) + '&type=0', null, KK_HEADERS_API, 'GET'));
  ...
  await Promise.all(results.map(async item => {
    try {
      const raw2=_parseBody(await API.launcherFetch(KK_DRAMA+item.id+'?id='+item.id, null, KK_HEADERS_API, 'GET'));
...
async function _loadEpList(kkItem) {
  ...
  if (!eps.length) {
    try {
      const raw=_parseBody(await API.launcherFetch(KK_DRAMA+kkItem.id+'?id='+kkItem.id, null, KK_HEADERS_API, 'GET'));
...
async function _renderPlayer(ep, epNum, kkItem, extra) {
  ...
  async function _applySub(video, sub) {
    ...
    _freeBlob();
    _log('[KissKH] _applySub: ' + sub.label + ' src=' + sub.src, 'info');
    const raw=await _fetchSubContent(sub.src);
    _log('[KissKH] sub content len=' + (raw ? raw.length : 'null'), raw ? 'info' : 'warn');
    if (!raw) return;
    let vtt=raw;
...
_log('[KissKH Plugin] v2.6.0 terdaftar', 'success');
